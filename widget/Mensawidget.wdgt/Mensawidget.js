...
function refreshFeed()
{
    if (!feed.url || feed.url.length < 1) {
        showMessageInContents(getLocalizedString("Kein Feed angegeben"),
            getLocalizedString("Bitte geben Sie eine gültige Feed-URL ein."));
        return false;
    }
...
function processFeedDocument(doc)
{
    ...
    if (doc) {
        ...
        if (results == null || results.length < 1) {
            showMessageInContents(getLocalizedString("Keine Objekte gefunden"),
                getLocalizedString("Der Feed enthält keine Einträge."));
            return;
        }
        ...
        if (results == null || results.length < 1) {
            showMessageInContents(getLocalizedString("Keine Objekte zum Anzeigen"),
                getLocalizedString("Der Feed enthält keine Einträge zu den angegebenen Kriterien."));
            return;
        }
        ...
        // document is empty
        showMessageInContents("Heute gibt es kein Mensaprogramm", "");
    }
}
...
function parseRSSFeed(rss)
{
    ...
    if (channel) {
		...
		var description = findChild(channel, "description");
		var heading = document.getElementById("heading");
		heading.innerHTML = allData(description);
    }
...
function showNewCount(newCount)
{
    setLoadingText(getLocalizedString("%s neu").replace("%s", newCount));
}
...
function startLoadingAnimation()
{
    ...
    {
        var loading = getLocalizedString("Laden");
...
function search(searchEvent)
{
    ...
    if (lastResults && lastResults.length) {
        ...
        if (searchResults == null || searchResults.length < 1) {
            showMessageInContents(getLocalizedString("Keine Objekte gefunden"),
                getLocalizedString("Keine passenden Objekte zu den Suchbegriffen gefunden."));
        }
        else {
            // Generate the display
...
function SetNewFeedURL()
{
	...
	var url = "http://wap.trivalg.de/" + mensa.value + "/" + tag.value + ".rss";
